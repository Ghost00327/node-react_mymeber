...
import { DELETE_SUB_FOLDER, GET_SCHEDULE_MAILS, GET_CATEGORIES_LIBRARY, changeFilter, DELETE_FOLDER } from "../../../../../redux/actions/library"
import { connect } from "react-redux"
import RenameFolder from "./renameFolderName"
import NewFolder from "./libraryCategoryModal"
...
class LibrarySidebar extends React.Component {
  ...
  CallChangeFilter = (_path, id) => {
    this.props.changeFilter(_path, id)
  }
  deleteFolder = (item) => {
                          ...
                          {collapseItem.folder?.map((subFolder, _i) => {
                            return (
                              <ListGroupItem
                                ...
                                active={`/app/marketing/email/library/${collapseItem.categoryName}/${subFolder.folderName}` === this.props.routerProps.location.pathname}
                                className="border-0 cursor-pointer pt-0"
                              >
                                <FolderPlus size={18} />
...
const mapStateToProps = state => {
  return {
    categories: state.EmailLibraryMarketing.categories,
    allScheduleMails: state.EmailLibraryMarketing.allScheduleMails,
  }
}
