...
class SideMenuContent extends React.Component {
  ...
  render() {
    ...
    const SchoolmenuItems = navigationConfig.SchoolNavigationConfig.map(item => {
    ...
    const AdminmenuItems = navigationConfig.AdminNavigationConfig.map(item => {
      const CustomAnchorTag = item.type === "external-link" ? `a` : Link
      // checks if item has groupheader
      if (item.type === "groupHeader") {
        return (
    ...
    return <React.Fragment>{ loggedinUser.data.role == 1 ? AdminmenuItems:SchoolmenuItems }</React.Fragment>
  }
}
